...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, labels=()):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        #     x = x[torch.isfinite(x).all(1)] #all(1)里的1意思为根据torch.isfinite(x)结果的第1维来判断
...
def  clip_coords(boxes, img_shape):
    ...
    # boxes[:, 0::2].clamp_(0, img_shape[1]-1)  # x1,x2
    # boxes[:, 1::2].clamp_(0, img_shape[0]-1)  # y1,y2
...
def coco80_to_coco91_class():  
    '''converts 80-index (val2014) to 91-index (paper) https://tech.amikelive.com/node-718/what-object-categories-labels-are-in-coco-dataset/'''
