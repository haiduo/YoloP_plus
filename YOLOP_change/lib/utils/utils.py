    ...
    if cuda:
        ...
        for i in range(0, ng):
            if i == 1:
                s = ' ' * len(s)
            if logger:
                logger.info("       CUDA:%g (%s, %dMB)" % (i, x[i].name, x[i].total_memory / c))
    else:
        if logger:
            logger.info(f'Using torch {torch.__version__} CPU')
    cuda_str = "cuda:" + device
    if logger:
        logger.info('Using GPU:%s\n'% (cuda_str) )  # skip a line
    return torch.device(cuda_str if cuda else 'cpu')
...
def save_checkpoint(epoch, name, model, optimizer, output_dir, filename, is_best=False):
    ...
    if is_best and 'state_dict' in checkpoint:
        torch.save(checkpoint['state_dict'], os.path.join(output_dir, 'model_best.pth'))
    else:
