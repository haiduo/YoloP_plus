...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            h0, w0 = img0.shape[:2]
            self.frame += 1
            print('\n video %g/%g (%g/%g) %s: %s' % (self.count + 1, self.nf, self.frame, self.nframes, path, img0.shape), end='')
        else:
            # Read image
            self.count += 1
        ...
        img = np.ascontiguousarray(img) # 等价 img = img[:, :, ::-1].transpose(2, 0, 1)
        # cv2.imwrite(path + '.letterbox.jpg', 255 * img.transpose((1, 2, 0))[:, :, ::-1])  # save letterbox image
        return path, img, img0, self.cap, shapes

    def new_video(self, path):
        self.frame = 0
        self.cap = cv2.VideoCapture(path) #生成读取视频对象
