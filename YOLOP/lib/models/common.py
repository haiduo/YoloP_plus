...
class Detect(nn.Module):
    ...
    def __init__(self, nc=8, anchors=(), ch=()):  # detection layer
        super(Detect, self).__init__()
        self.nc = nc  # number of classes
        self.no = nc + 5  # number of outputs per anchor 17
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                # right = self.anchor_grid[i].view(bs, self.na, 1, 1, 2)
                y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i].to(x[i].device)) * self.stride[i]  # xy
                # y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * right  # wh
    ...
    #         x[i]=x[i].view(bs, self.na, self.no, ny*nx).permute(0, 1, 3, 2).view(bs, self.na, ny, nx, self.no).contiguous()
    #         if cfg.MODEL.NOT_INIT_MODEL[0]:
    #             if not self.training or cfg.TRAIN.PLOT:  # inference
    #                 if self.grid[i].shape[2:4] != x[i].shape[2:4]:
    #                     self.grid[i] = self._make_grid(nx, ny).to(x[i].device)
    #                 y = x[i].sigmoid()
    #                 y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i].to(x[i].device)) * self.stride[i]  # xy
    ...
    #                 z.append(y.view(bs, -1, self.no))
    #     if self.training and not cfg.MODEL.NOT_INIT_MODEL[0] :
    #         return x
    #     else:
    #         if self.training:
    #             if cfg.TRAIN.PLOT :
