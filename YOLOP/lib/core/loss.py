...
class MultiHeadLoss(nn.Module):
    ...
    def forward(self, predictions, targets, shapes, model):
        ...
        _, _, height, width = targets[1].shape
        ...
        metric.reset()
        metric.addBatch(lane_line_pred.cpu(), lane_line_gt.cpu()) #仅考虑车道线的IOU损失
...
def get_loss(cfg, device):
    ...
    get MultiHeadLoss
    Inputs:
    -cfg: configuration use the loss_name part or function part(like regression classification)
...
class FocalLoss(nn.Module):
    '''
    Wraps focal loss around existing loss_fcn(), i.e. criteria = FocalLoss(nn.BCEWithLogitsLoss(), gamma=1.5, alpha=0.25)
